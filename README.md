...
회사 웹을 **컴포넌트 조합으로 만드는** React UI 라이브러리의 초기 기반입니다.
React 19 · TypeScript · SCSS · Vite. 토스의 공개 디자인 패턴을 참고한 자체 컴포넌트입니다. Tailwind, CSS-in-JS, 별도 상태 관리, 모노레포 도구를 사용하지 않습니다.
...
터미널에 표시된 주소에서 컴포넌트 카탈로그, 자산 홈, 설정, 결제 예제와 소스 코드를 확인합니다.
...
```text
src/components/     레이아웃, 타이포그래피, 폼, 표면 컴포넌트
...
- **브라우저 기본 동작**: 폼은 네이티브 요소를 사용합니다. Dialog, Menu, Combobox 등 복합 상호작용은 접근성 요구를 검토한 후 검증된 headless primitive 도입을 판단합니다.
...
현재는 22개 컴포넌트와 3개 화면 예제를 포함한 초기 기반입니다. 전체 UI 카탈로그, MCP 서버, 레지스트리 게시, 브라우저 접근성 전수 검증까지 완료된 제품은 아닙니다.
...
토스 스타일 변경 기준과 참고 자료는 [디자인 방향](docs/design-direction.md)에 정리했습니다. 공식 TDS 패키지나 API 호환 구현은 아닙니다.
