...
Mega UI의 색·치수·타입 스케일은 한국어 웹 서비스의 읽기 리듬과 밀도를 기준으로 Mega UI가 정한 값입니다.
API 이름, props, 클래스 이름은 모두 Mega UI의 것입니다.
다른 회사의 로고나 그래픽 자산은 사용하지 않고, 예제 데이터도 전부 가상입니다.
...
버튼 높이는 데스크톱 사다리 **24 / 28 / 34 / 40**px에, 모바일 CTA용 **56**px을 더한 5단계입니다.
...
제목 5단계와 본문 4단계 역할을 둡니다. 굵기는 400/500/600/700만 사용합니다(가변 폰트라도 800은 쓰지 않습니다).
...
수치는 Mega UI가 정한 값이며, 다른 디자인 시스템과의 색상·치수·API 호환을 목표로 하지 않습니다.
